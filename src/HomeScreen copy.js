...
import {ViewCategory} from './reducers/CategorySlice';
const HomeScreen = props => {
  const {navigation} = props;

  const handleNavigateToDetailScreen = (crop, plantpots, plantingtools) => {
    navigation.navigate('DetailScreen', {crop, plantpots, plantingtools});
  };
  ...
  const categoryData = useSelector(state => state.view);
  ...
  const renderItemCrops = () => {
    ...
    for (
      let i = 0;
      i < Math.min(productsData.length, categoryData.length, 4);
      i += cropsPerRow
    ) {
      const rowItems = [];
      ...
      for (let j = 0; j < cropsPerRow; j++) {
        const index = i + j;
        if (index < Math.min(productsData.length, categoryData.length, 4)) {
          rowItems.push(
            <TouchableWithoutFeedback
              key={productsData[index]._id}
              onPress={() => handleNavigateToDetailScreen(productsData[index])}>
